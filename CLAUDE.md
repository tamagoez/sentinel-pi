...
### 76. 定時処理の動画生成が「動いていない」— h264_v4l2m2m のハング・`_reboot()` の戻り値未確認・帯の二重再エンコードをまとめて直した

「映像生成機能が動いていません。しっかり動くようにし、毎日の生成と
再起動がされていないのも含め直してください」という報告があった。ログを
新たに採取する前の時点だったため、`modules/maintenance.py` を読み直して
見つかる範囲の実装上の欠陥を洗い出し、まとめて潰した。

#### 再起動が起きない本当の原因: `_reboot()` が戻り値を確認していなかった
...
`subprocess.run()` は `check=True` を渡さない限り、コマンドが非ゼロ終了
しても例外を投げない。`sudo -n /sbin/reboot` が (sudoers の記述と実際の
バイナリパスがずれている、`sudo` が PATH に無い、等の理由で) 失敗して
非ゼロで終了しても、この関数はそれを検知できず、`return` して**成功した
ものとして扱い、ログにも一切残さず終わっていた**。`core/state.py` の
`_apply_governor()` と `modules/hotspot.py` の `set_ssid()` はどちらも
同じ「sudo 経由でスクリプトを呼ぶ」パターンでありながら `returncode` を
きちんと確認して警告ログを出している — この関数だけがそれを欠いていた。
2 番目のフォールバックも `sudo -n` を欠いており、`/etc/sudoers.d/
sentinel` が実際に許可しているのは `sudo -n systemctl reboot` であって
素の `systemctl reboot` ではないため、非 root の `sentinel` ユーザーからは
そもそも失敗する運命だった。

`returncode` を確認し、非ゼロなら次のフォールバックへ進みながら都度
`log.warning()` を残し、フォールバック 2 番目にも `sudo -n` を付けるよう
直した (CLAUDE.md #4 が定めた「reboot だけを個別に許可する」sudoers の
実際のエントリと一致させた)。**この戻り値チェックを外して「呼べたら
成功」という実装に戻さないでください** — 同じ「定時処理は完了ログが
出るのに Pi が再起動しない」不具合に戻ります。

#### 動画生成が「動いていない」本当の原因: `h264_v4l2m2m` が固まりうる

`_encoder_args()` は ffmpeg が `h264_v4l2m2m` (ハードウェアエンコーダ)
を報告すれば無条件に優先していた。Web で調べ直したところ、この
エンコーダは固定ビットレート (CBR) 指定と組み合わさると
`VIDIOC_STREAMON failed` で失敗する既知の不具合を持ち
([Raspberry Pi Forums](https://forums.raspberrypi.com/viewtopic.php?t=330999))、
さらに Pi 3B+ 実測でも 1 コアフル稼働の libx264 と大差ない速度しか出ない
([Raspberry Pi Forums](https://forums.raspberrypi.com/viewtopic.php?t=353958))
と分かった。「動かないリスクを取ってまで選ぶ利点が無い」うえ、失敗の
仕方によっては V4L2 デバイスをカーネル内で D-state のまま掴み、
呼び出し元の ffmpeg プロセスが `SIGKILL` にも応答しなくなることがある
— `subprocess.run(..., timeout=...)` は超過時に `kill()` を送るだけなので、
この状態になると `_run()` 自身のタイムアウト機構ごと無力化され、定時
処理がそのステップで実質的に永久停止する。これは CLAUDE.md 全体で
繰り返してきた「ドライバ依存の機能をソフトウェア側の確実な実装に置き
換える」判断 (drawtext→Pillow、asound.conf→sysdefault、bluetoothctl
スクレイピング→dbus-next 等) と同じ形の問題だったため、同じ判断を適用
した。**h264_v4l2m2m の検出・優先を復活させないでください** — 同じ
「生成がどこかで止まって二度と進まない」不具合に戻ります。

#### 「一度詰まると永久に直らない」クラスの不具合への一般的な対策

`h264_v4l2m2m` を廃止すれば主要因は消えるはずだが、CLAUDE.md #59 が
一度踏んだとおり「`STATE["running"]` が何らかの理由で True のまま固定
されると、毎日 4 時の `loop()` が『すでに実行中です』で永久に空振り
し続け、動画生成も (`run_now()` の中でしか判定されない) 再起動も二度と
起こらなくなる」というクラスの不具合は、原因が変わっても同じ形で再発
しうる。今回は原因を 1 つ塞いだだけで終わらせず、この不具合クラス自体
に対する防御を `run_now()` に追加した。

- **前段の停止処理 (`music.PLAYER.persist`/`stop`・`camera.shutdown`)
  と `_build()` 全体の両方に `asyncio.wait_for()` の上限を掛けた**
  (`_PRESTOP_STEP_TIMEOUT`=30 秒、`_BUILD_TIMEOUT`=40 分)。
  `asyncio.to_thread()` が包むスレッド自体は外部から強制終了できない
  (asyncio の既知の制約) が、`wait_for` はタイムアウト時に**待っている
  コルーチン側を確実に先へ進める** — 裏のスレッドが本当に固まっていても、
  `run_now()` の `finally: STATE["running"] = False` には確実に到達する
  ため、その日の再起動判定まで進めるし、翌日の定時実行も塞がれない。
  スタブ化した `_build` を実際に固まらせて `run_now()` を 2 回連続で
  呼ぶテストで、1 回目がタイムアウトで失敗扱いになったあと
  `STATE["running"]` が確実に `False` に戻り、2 回目が正常に実行できる
  ことを確認済み。
- **それでも `STATE["running"]` が長時間 True のまま観測された場合
  (`_STALE_RUN_SECONDS`=2 時間) の保険**として、次に `run_now()` が
  呼ばれたときに「放棄された実行」とみなし、居座っている ffmpeg を
  `pkill -9 -f ffmpeg` で掃除してから新しい実行を始めるようにした。
  この Pi で ffmpeg を使う処理は定時処理以外に存在しないため、無差別な
  `pkill` でも他機能を巻き込む心配がない。

**この 2 つの防御 (wait_for の上限、stale-run の回収) を外さないで
ください** — h264_v4l2m2m 以外の未知の原因で同種の永久停止が再発しても、
今回のように「原因を 1 つずつ潰すまで直らない」状態へ逆戻りします。

#### 「URL 表示が映像に間に合うように爆速で」— タイルと帯を 1 回の ffmpeg 呼び出しに統合

以前は「カメラ映像をタイルに統合する (`_tile()`)」と「時計/アクセスログ
の帯を下部に重ねる (旧 `_overlay_info_band()`)」がそれぞれ独立に、
最終解像度の映像を**フル尺で 1 回ずつ**再エンコードしていた — 定時処理
1 回につき、タイル映像の全長を都合 2 回エンコードすることになり、
Pi 3B+ のソフトウェアエンコードではこれが生成時間の大きな割合を占めて
いた。帯 (`_build_info_band()` が作る、時計とその瞬間のアクセス先を
表示する短い映像) を、タイルを組み立てる `xstack` フィルタの出力へ
`filter_complex` の中でそのまま `overlay` へつなげるだけで、この 2 回を
1 回に減らせる。`_tile()` を拡張してこの合成を担わせ、`_overlay_info_
band()` は削除した。実際に ffmpeg を動かして検証したところ、出力の
解像度は「タイル映像とちょうど同じ高さ」のまま (帯の分だけ映像が
縦に伸びるのではなく、映像の下端に重なる) で、フレームごとの時計・
アクセス先も正しい実時刻に同期して切り替わることを確認した。**この
2 回のフルサイズ再エンコードへ戻さないでください** — 「URL 表示を
爆速で」という要望が指していた生成時間の大部分がここにあった。

なお、URL 表示自体の方式 (スクロールか、瞬間ごとの静止表示か) は #14
の判断のまま変えていない — #14 の時点で「スクロールは 1 件あたりの
表示時間が短すぎて読めない」と分かった上での選択であり、今回の要望
(「いずれにせよ爆速で」) は表示方式そのものではなく、生成が映像の尺に
対して遅れずに間に合うことを求めていたため、上記のエンコード回数削減で
対応した。

#### 時計/URL の帯が消える経路を診断可能にした

`_can_render_text()` (Pillow または対応フォントが無い環境では False を
返す) が False のとき、NODATA 表示・カメラ名キャプション・時計/URL の
帯のすべてが**ログに一切残さず静かに省略**されていた。「動画は生成
されるが時計も URL も出ない」という別の症状として報告されたときに、
原因の切り分けに使える手がかりが無い状態だったため、`_build()` の先頭
で一度だけ警告ログを出すようにした (build 呼び出しのたびに 1 回だけ —
`_render_text_png()` は 1 回の build で何度も呼ばれるため、そのたびに
出すとログが埋もれる)。`bootstrap.sh` は `python3-pil`/`fonts-dejavu-
core`/`fonts-noto-cjk` を導入済みのはずなので、実機でこの警告が出る
場合は `update.sh` の再実行を案内すればよい。**この診断ログの追加は
...
### 81. Bluetooth 出力へ切り替えても AUX ミュートのまま止まり続ける不具合、Bluetooth 出力中に時報のチャイムが途切れる不具合、Bluetooth ページの音量表示が「-」に戻る不具合を修正

「Bluetooth接続しているのに、AUXモードのためBGM停止になってしまう」
「Bluetooth接続時に、時報の読み上げが始まると、効果音が止まってしまう」
という 2 つの報告があった。どちらも #73/#77 で構築した「AUX とBluetooth
出力は完全に独立した経路」という設計自体は壊れていない — 実際に踏んだのは
経路の**切り替えタイミング**と**Bluetooth 出力の同時オープン制約**という、
別々の抜けだった。

#### BGM が「Bluetooth に繋いだのに AUX ミュートのまま止まる」

`music_mute_bgm_on_aux` (#77) は、実効出力先が AUX のときだけ `play()` が
mpg123 を spawn する前に即座に `playing=False`/`suspended_by="aux_muted"`
で return する、という仕組みだった。この分岐が発火した時点では **mpg123
のプロセスは一度も起動していない** (`self.proc` は `None` のまま)。

`bluetooth.output_loop()` が接続を検知して `music.set_bt_output()` を呼ぶと、
`bt_output_addr` は正しく更新されるが、続く再評価は
`running = PLAYER.proc is not None and PLAYER.proc.poll() is None` が真の
場合、つまり**すでに mpg123 が動いている場合だけ**行われていた。AUX
ミュートで止まっていたケースは `proc is None` なので `running=False` —
`bt_output_addr` が変わったことも、ミュートし続ける理由 (AUX 出力) が
もう無くなったことも、誰も再評価しなかった。結果、Bluetooth へ接続しても
BGM は「AUX だったときのミュート」を引き継いだまま永久に止まり続けていた。

修正は、`PLAYER.suspended_by == "aux_muted"` (この mute 分岐だけが設定する
値、他の停止理由 — eco・Bluetooth 受信の退避・手動停止 — とは混同しない)
を目印にした 3 つ目の分岐を `set_bt_output()` に追加したことである。
`running` が偽でも、**直前まで AUX ミュートで止まっていて、かつ新しい
出力先が Bluetooth (`addr` が真値)** の場合だけ `PLAYER.play()` を呼んで
再評価する。`play()` 自身は `bt_output_addr` が立っているかどうかで
mute するかどうかを判定する関数なので、ここで「無条件に再生を始める」
わけではない — 「ミュートを続ける理由 (AUX 出力) がもう無いので、通常の
再開判定に戻す」だけである。逆方向 (Bluetooth → AUX) は既存の
`if running:` 分岐がそのまま扱う — 再生中に AUX へ戻ればそこで `play()`
が再度呼ばれ、`music_mute_bgm_on_aux` が有効なら正しくミュートし直される。
**この `was_aux_muted` 判定を外して `if running:` だけに戻さないでください**
— 同じ「Bluetooth に繋いでも AUX ミュートのまま止まり続ける」不具合に
戻ります。手動でユーザーが停止した場合 (`suspended_by` が `"aux_muted"`
以外、または空文字) はこの分岐を通らないため、出力先を変えただけで
勝手に再生が始まることはない。

#### Bluetooth 出力中、時報のチャイムが読み上げ開始と同時に途切れる

`voice._speak_sync()` は `chime=True` のとき、`_play_chime()` を
`subprocess.Popen` で非同期に開始し、**その直後に** TTS (`open_jtalk`/
`espeak-ng` → `aplay`) を同じ `device` へ向けて再生し始める設計だった
(CLAUDE.md #58)。AUX (`sysdefault:CARD=<N>`、alsa-lib の dmix) はこの
「同じデバイスへ 2 つのクライアントが同時に書き込む」を構造的に受け付ける
ため、これで正しく重なって鳴る。しかし Bluetooth 出力
(`bluealsa:DEV=<MAC>,PROFILE=a2dp`) の A2DP ソース PCM は
**同時に開けるクライアントが 1 つだけ**という制約を持つ (music.py の
`pause_for_voice()`/`resume_after_voice()` が BGM とアナウンスを同時に
開かせず一時停止で調停しているのと全く同じ理由、CLAUDE.md #73)。この
制約を知らずに `_play_chime()` と TTS の `aplay`/`mpg123` を同じ
Bluetooth デバイスへ並行に開こうとすると、後から開いた側が PCM を奪い、
先に鳴り始めていたチャイムが読み上げの開始と同時に途切れる — 報告どおりの
症状である。

修正は、`device` が Bluetooth 出力 (`bluealsa:` で始まる文字列) かどうかを
`_speak_sync()` の先頭で判定し、

- **AUX の場合**: 従来どおり `_play_chime()` を非同期に開始し、その直後に
  TTS を始める (重ねて鳴らす、変更なし)。
- **Bluetooth の場合**: `_play_chime()` を呼んだ直後に `proc.wait()` して
  **チャイムが完全に鳴り終わるまで TTS を始めない** (重ねない、順番に
  鳴らす)。

とした。**この判定・分岐が渡す `device` 文字列そのものは一切変更していない**
— `_play_chime()`/TTS のどちらも、呼び出し元 (`_device()` =
`music.current_output_device()`) が返した実際の出力先へそのまま書き込む。
Bluetooth 出力中にチャイムや読み上げを AUX へ流す、あるいは AUX 出力中の
音量設定 (`_aux_volume_max()`/`music_volume_boost_enabled`、#80) を
Bluetooth 側の再生に適用する、といった経路はどこにも増えていない —
変えたのは「同時に開くか、順番に開くか」という実行順序だけである。
**この Bluetooth 判定を外して常に並行再生する実装に戻さないでください**
— 同じ「読み上げが始まると効果音が途切れる」不具合に戻ります。逆に
AUX でも常に順番に鳴らす実装に変えないでください — dmix はそもそも
同時再生が可能なので、律速する理由が無く「重ねて鳴らす」という #58 の
要望に反します。

両方の修正について、実際の `Player`/`_speak_sync` のコード (mpg123 の
spawn やサブプロセスの起動を実行しない、イベント順序だけを検証するスタブ)
を使い、(1) AUX ミュート中に Bluetooth 出力へ切り替えると `play()` が
再評価されること、(2) 手動停止中は同じ切り替えで勝手に再生が始まらない
こと、(3) AUX ではチャイムと TTS が重なって開始されること、(4) Bluetooth
では TTS の開始前にチャイムの完了を待つこと、の 4 点を確認済みである。

#### Bluetooth ページの音量表示が数秒後に「-」へ戻り、設定できない

`renderBluetooth()` (WebSocket 更新のたびに、数秒おき・間引きなしで呼ばれる、
#79 で既に扱った関数) は接続中の端末の音量スライダーを毎回**固定の
テンプレート文字列**で組み立てていた — スライダー本体 (`#bt-vol`) には
`value` 属性が無い (ブラウザの既定値である中央=50 になる) まま、表示側
(`#bt-vol-v`) も常に `"-"` を書いていた。実際の値は `fetchBtVolume()`
(接続直後に一度だけ呼ばれる) か `btSetVolume()` (スライダー操作時) が
DOM を直接書き換えて反映していたが、この 2 つはどちらも「今表示されている
DOM を後から上書きする」だけで、**次に `renderBluetooth()` が呼ばれた
瞬間、テンプレートの `"-"`/属性無しのスライダーがその上書きをそのまま
消してしまう**。ユーザーがスライダーからフォーカスを外した (クリックで
他の場所へ移る、モバイルでタップし直す等) 次の WebSocket 更新で確実に
再現し、「音量を変えても数秒でまた「-」に戻る、設定できているように
見えない」という報告になっていた。これは #79 (ペアリング済み端末一覧が
「読み込み中」に戻る) と全く同じ構造の不具合が、同じ Bluetooth ページの
別の要素で新たに発生したものである。

修正は #79 と同じ考え方 — 「最後に確定した値をキャッシュしておき、毎回の
再構築でもそのキャッシュを使う」— を音量にも適用したことである。
`BT_VOL_LAST` (直近の音量、初期値 `null` = 未取得) を新設し、
`fetchBtVolume()`/`btSetVolume()` の両方がこの値を更新するようにした上で、
`renderBluetooth()` のテンプレート自身も `BT_VOL_LAST` から
`value="${BT_VOL_LAST ?? 100}"`/`${BT_VOL_LAST != null ? BT_VOL_LAST + "%" : "-"}`
を組み立てるよう変更した。これにより、`renderBluetooth()` が何度呼ばれても
直近に確定した値がそのまま維持される。接続端末が変わった・切断された
場合は `BT_VOL_LAST = null` に戻す (別端末の音量を誤って表示し続けない
ため) — この 2 か所は #79 で `BT_VOL_ADDR` をリセットしていた分岐と
同じ場所に追加した。**このキャッシュを外して固定テンプレート文字列
(`"-"`、`value` 属性無し) に戻さないでください** — 同じ「音量が数秒で
「-」に戻り、設定できない」不具合に戻ります。

**この 3 つの修正のいずれも、Bluetooth 出力の音量/デバイス文字列を AUX
側の経路 (`_aux_volume_max()`/`music_volume_boost_enabled`、#80) へ
混ぜていない。** #77/#80 が確立した「Bluetooth 出力の音量は常に 0-100、
AUX の上限解除の対象外」という分離は、この節のどの修正でも変更していない
— 3 つとも「正しい経路の呼び出し忘れ/再評価漏れ」「同じ PCM への同時
オープン」「UI の再描画による表示の巻き戻り」という、経路の分離そのものと
は別の層の不具合だった。
