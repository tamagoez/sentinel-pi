...
**これは自動でも行われます。** `sentinel-autoupdate.timer` が 30 分ごとに
git リモートを確認し、新しいコミットがあれば自分で `update.sh` を実行します
— 毎回マージのたびに上のコマンドを手動で叩く必要はありません。ほとんどの
周期では軽い `git fetch` だけで終わり (実際に新しい変更がない限り何もしません)、
作業ツリーに未コミットの変更がある場合は何もしないので、機体上で手を
入れている最中に横から上書きされることもありません。履歴は
`journalctl -t sentinel-autoupdate` で確認できます。
