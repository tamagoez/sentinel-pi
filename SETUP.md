...
- **H5 — AdGuard Home.** DietPi pre-configures it, so **there is no setup
  ...
  and change the password if you want to. Afterwards port 8083 is blocked
  from the LAN/hotspot; run `sudo sentinel-adguard-8083 enable [MINUTES]`
  on the Pi to open it again for a while.
