...
# 100 を超える値も受け付ける (ソフトウェア側の増幅、README.remote に
# "VOLUME/V <percent>: set volume in % (0..100...)" と明記 — 上限は
# ドキュメント化されていない) が、無制限に許すと歪み・ヘッドホン使用時の
# 難聴リスクが跳ね上がるため、上限そのものは動かさず固定値にしている。
# 300 という値は思いつきの数字ではなく、mpg123 自身がイコライザー実装
# (CLAUDE.md #72) で採用している実用上限 (`-f` 初期スケールの推奨範囲
# "values work best between 0.00 and 3.00"、線形ゲイン 3.0 = 300%) と
# 揃えている — mpg123 自身が「ここまでなら歪みが実用的な範囲に収まる」と
# 案内している値を、AUX の音量上限としてもそのまま踏襲した。「小さい
# スピーカーで聞こえない」という報告を受けて 150 から引き上げた際も、
# この数字自体は変えていない。**この上限自体を利用者が変更できる設定に
# しないでください** — 「限界を超えて設定できるように」という要望自体が
# 「安全な上限をどこかに置いた上での解放」を意図したものであり、上限を
# 無くす/利用者が更に引き上げられるようにすることは意図と異なる。
_AUX_VOLUME_BOOST_MAX = 300
    ...
    def status(self) -> dict:
        with self._lock:
            ...
            return {
                ...
                # スライダーが実際に到達できる上限。AUX かつ
                # music_volume_boost_enabled が有効なときだけ 100 を超える
                # (_aux_volume_max())。Bluetooth 出力機器はこの解除の対象外
                # で常に 100 — 判定をここに集約し、index.html 側では
                # 出力先/boost フラグを見て条件分岐させない (CLAUDE.md #80、
                # #m-vol の max 属性がハードコード 100 のままだと boost を
                # 有効にしてもスライダーからは絶対に到達できなかった不具合の
                # 修正)。
                "volume_max": _aux_volume_max() if not self.bt_output_addr else 100,
                "shuffle": bool(config.get("music_shuffle")),
                "repeat": str(config.get("music_repeat")),
                "seed": self.seed,
